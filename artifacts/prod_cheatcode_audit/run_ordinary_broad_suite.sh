...
audit_dir="${1:?pass host output directory}"
audit_dir="$(realpath "$audit_dir")"
name="${AIC_AUDIT_CONTAINER_NAME:-aic_ordinary_broad_cheatcode_20260923}"
...
cp "$here/official_qualification/capture_observations.py" "$audit_dir/capture_observations.py"
printf '%q ' "$0" "$@" > "$audit_dir/host_command.txt"
printf '\n' >> "$audit_dir/host_command.txt"

if docker inspect "$name" >/dev/null 2>&1; then
  echo "Container $name already exists; inspect it before restarting this suite" >&2
  exit 1
fi
...
if test -f "$audit_dir/engine.exit"; then
  echo "Engine exited before policy startup; inspect $audit_dir/engine.log" >&2
  exit 1
fi
...
docker exec -d "$name" bash -lc '
  ...
  ros2 run aic_model aic_model --ros-args -p use_sim_time:=true \
    -p policy:=aic_example_policies.ros.CheatCode > /audit/cheatcode.log 2>&1
'
